...
def get_resnet_18(path_state_dict, device, vis_model=False):
    ...
    :return:
    """
    model = models.resnet18()#torchvision中的models
    if path_state_dict:#如果有参数
        pretrained_state_dict = torch.load(path_state_dict)
        model.load_state_dict(pretrained_state_dict)#把训练好的参数加载到model中 所以这一步完了就训练好了因为权重有了
    model.eval()#test的时候必须要加 不启用 BatchNormalization 和 Dropout
    #train的时候加：Model.train(mode=True)启用 BatchNormalization 和 Dropout

    if vis_model:
        print("########summary(model)#######")
        from torchsummary import summary
        summary(model, input_size=(3, 224, 224), device="cpu")#模型的一个可视化 就是输出的那一堆模型结构
        print("############################")
    model.to(device)
    return model
...
def get_resnet_50(path_state_dict, device, vis_model=False):
    ...
    if path_state_dict:
        pretrained_state_dict = torch.load(path_state_dict)#预训练权重加载
