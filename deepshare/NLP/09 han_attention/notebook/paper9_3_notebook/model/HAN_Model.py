...
class HAN_Model(nn.Module):
    ...
    def forward(self, x,gpu=False):
        ...
        word_outputs = self.word_dense(word_outputs)
        weights = torch.matmul(word_outputs,self.word_context)
        ...
        sentence_outputs = self.sentence_dense(sentence_outputs)
        weights = torch.matmul(sentence_outputs,self.sentence_context)
        weights = F.softmax(weights,dim=1)
